...
### Avantages
- **Prévention des conflits** : Évite la surcharge accidentelle de données de session
- **Intégrité des données** : Assure la cohérence de l'historique de jeu
- **UX améliorée** : Messages d'erreur clairs pour les utilisateurs
- **Maintenance simplifiée** : Réduction des problèmes de données corrompues

### Tests unitaires
La fonctionnalité est validée par le test `test_start_scenario_prevents_duplicate_session()` dans `back/tests/routers/test_scenarios.py`, qui :
1. Créé une session initiale
2. Tente de créer la même session
3. Vérifie le retour du code HTTP 409
4. Valide le message d'erreur approprié
