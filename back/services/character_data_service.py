    ...
    def load_character(self, character_id: Optional[str] = None) -> Character:
        ...
        **Lève:** FileNotFoundError si le personnage n'existe pas
        ...
        if is_incomplete:
            ...
            state_data["status"] = CharacterStatus.IN_PROGRESS
            log_debug("Personnage incomplet détecté", 
                     action="load_character_incomplete", 
                     character_id=target_id)
        
        try:
            character = Character(**state_data)
            log_debug("Personnage chargé avec succès", 
                     action="load_character", 
                     character_id=target_id)
            return character
        except Exception as validation_error:
            log_debug("Erreur de validation lors du chargement", 
                     action="load_character_validation_error", 
                     character_id=target_id,
